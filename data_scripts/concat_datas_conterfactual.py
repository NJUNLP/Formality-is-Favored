...
def create_data(args):
    ...
    with open("./subdata_items/templates_counterfactuals.json", 'r', encoding='utf8') as f:
        counterfactuals_template = json.load(f)
    ...
    for bio_idx, name in enumerate(name_list):
        ...
        cur_data["conterfactual_type_info"] = {
            "type_name": "conterfactual_type_info",
        ...
        # second type
        for temp_idx in range(len(selected_numbers)): 
            template = counterfactuals_template[temp_idx].split("Template:\n")[-1]
            if args.fullname:
                template = template.replace(" she ", fullname + " ").replace("She ", fullname + " ")
                template = template.replace(" he ", fullname + " ").replace("He ", fullname + " ")
            one_bio_text = template.replace("<full name>", fullname).replace("<birth date>", cur_data["conterfactual_type_info"]["birth_date"]).replace("<birth place>", cur_data["conterfactual_type_info"]["birth_place"]).replace("<university name>", cur_data["conterfactual_type_info"]["university"]).replace("<major>", cur_data["conterfactual_type_info"]["major"]).replace("<work company>", cur_data["conterfactual_type_info"]["company"]).replace("<work place>", cur_data["conterfactual_type_info"]["workplace"])
            assert "<" not in one_bio_text and ">" not in one_bio_text, one_bio_text
            cur_data["text_result"].append(one_bio_text)
    ...
    with open("./type_fights/bio_data_train_general_vs_counterfactuals.json", 'w', encoding='utf8') as f:
        json.dump(train_data, f, indent=4)

    with open("./type_fights/jsonl_bio_data_train_general_vs_counterfactuals.json", 'w', encoding='utf8') as f:
        data_list = []
